(()=>{
  ...
  function build(detail){
    ...
    const gameKeywordPool=['唤醒体','守密人','界域','命轮','灵知','融灾','密契','理智','狂气','调查员','启灵','同调率','污染','深潜','弥萨格','黑池','裂隙','卡达斯','旧日星门','深海回声','资源整备','阵线','观测','记忆残响','理性校准','命轮共鸣','界域适应','灵知回路','融蚀','梦境记录'];
    const cthulhuKeywordPool=['旧日','深海','裂隙','不可名状','古神','外神','星辰排列','潮汐','雾海','梦境','低语','黑潮','深潜','理智边界','血肉祭坛','星门','沉睡者','禁忌知识','异界注视','疯狂回声','远古印记','虚空航线','梦魇','深渊','失落海港','未知仪式','暗潮','星尘','古老契约','不可见风暴'];
    const keywords=[gameKeywordPool[(seed>>>7)%gameKeywordPool.length],gameKeywordPool[(seed>>>12)%gameKeywordPool.length],cthulhuKeywordPool[(seed>>>17)%cthulhuKeywordPool.length],cthulhuKeywordPool[(seed>>>22)%cthulhuKeywordPool.length]].filter((x,i,a)=>a.indexOf(x)===i);
