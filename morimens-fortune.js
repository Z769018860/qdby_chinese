(()=>{
  ...
  function build(detail){
    ...
    const cleanAvatarQuotes=avatarQuotes.filter(x=>!String(x).includes('限时纪行'));\n    const avatarCaption=cleanAvatarQuotes[(seed>>>21)%cleanAvatarQuotes.length]||'「不允许你们靠近守密人半步……！」';
    const cthulhuSign=cthulhuSigns[(seed>>>13)%cthulhuSigns.length]||themedText;
    const signText=`${avatarCaption}\\n${cthulhuSign}`;
