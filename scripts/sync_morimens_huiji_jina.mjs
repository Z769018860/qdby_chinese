...
const UA='qdby-chinese-morimens-sync/1.2';
...
function buildIndex(records){const map=new Map();for(const r of records){for(const key of [r.name,r.slug,r.assetSlug,...(r.aliases||[])]){const n=normalize(key);if(n)map.set(n,r)}}return map}
function match(profile,index){for(const value of [profile.englishName,profile.name]){const n=normalize(value);if(n&&index.has(n))return index.get(n)}const aliases={jenkin:'jenkins'};const n=normalize(profile.englishName);return aliases[n]?index.get(aliases[n])||null:null}
...
function targetsFromAwakenerIndex(md,skeydbRecords){
  ...
  for(const rec of skeydbRecords){
    if(normalize(rec.name)==='24'){targets.push({title:'「24」',rec,method:'special'});used.add('「24」');continue}
    ...
    if(title){targets.push({title,rec,method:'awakener-index'});used.add(title)}
  }
  return targets;
}
...
try{
  ...
  console.log(`Mapped ${targets.length}/${(skeydb.records||[]).length} SKeyDB awakeners from Huiji's awakener index.`);
  if(targets.length<40){
    ...
    for(const title of categoryTitles(category)){if(existing.has(title))continue;targets.push({title,rec:null,method:'category'});existing.add(title)}
  }
...
for(const target of targets){
  ...
  try{
    ...
    const rows=markdownRows(md),profile=profileFrom(rows,title,md),linked=target.rec||match(profile,index);
    records.push({skeydbId:linked?.id||null,ingameId:linked?.ingameId||null,slug:linked?.slug||null,name:title,englishName:profile.englishName||linked?.name||'',profile,voiceLines:voiceLinesFrom(rows),skillTables:skillTablesFrom(rows),summary:clean(md).slice(0,10000),source:{url:`${WIKI}/wiki/${encodeURIComponent(title)}`,mode:'jina',mapping:target.method}});
  }catch(error){failed.push({title,skeydbId:target.rec?.id||null,error:String(error)})}
  await new Promise(r=>setTimeout(r,900));
}
const bySkeydbId=Object.fromEntries(records.filter(r=>r.skeydbId).map(r=>[r.skeydbId,r]));const unmapped=records.filter(r=>!r.skeydbId).map(r=>({name:r.name,englishName:r.englishName||''}));
...
await saveJson(`${OUT_DIR}/zh-CN.json`,payload);await saveJson(`${OUT_DIR}/manifest.json`,{source,status:'ok',counts:{targets:targets.length,pages:records.length,mapped:payload.mapped,unmapped:unmapped.length,failed:failed.length},paths:{zhCN:`${OUT_DIR}/zh-CN.json`}});
