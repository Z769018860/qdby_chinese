...
try{
 ...
 for(const pair of ranges){
  const min=pair[0],max=pair[1],sliders=page.locator('[role="slider"]');
  if(await sliders.count()<2) throw new Error('未找到分段滑块');
  for(const item of [[0,min],[1,max]]){const s=sliders.nth(item[0]);await s.focus();await s.press('Home');for(let v=1000;v<item[1];v+=100)await s.press('ArrowRight');}
  const query=page.getByRole('button',{name:'查询'}); if(await query.count()) await query.click();
