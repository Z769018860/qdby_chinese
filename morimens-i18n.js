...
'目前伤害计算器已进入测试，欢迎反馈，sp小骑士已更新':'The damage calculator is now in testing. Feedback is welcome; SP Little Knight has been updated.',
'数据来源：':'Data sources: ',
'Eremora 网站':'Eremora website',
'给喜欢的唤醒体点赞，也可以点踩。榜单按':'Like or dislike your favorite Awakeners. The ranking is sorted in real time by ',
'赞 − 踩':'likes − dislikes',
'的净分实时排序，颜色深浅表示当前净分热度。':' net score; color intensity shows the current net-score heat.',
'正向热度':'Positive heat',
'负向热度':'Negative heat',
'守密人留言板':'Keeper Guestbook',
'匿名可用':'Anonymous posting available',
'昵称 / 邮箱非必填':'Nickname / email optional',
'最多 300 字':'Up to 300 characters',
'最新留言优先':'Newest first',
'正在连接留言板……':'Connecting to the guestbook…',
'打开原留言板':'Open the original guestbook',
'统一搜索配队与榜单统计的数据口径：当前期按 UID 合并基础缓存与 Top500 增量，并同步最新排名；角色身份统一映射到 SKeyDB canonical ID，修复同一角色因游戏 ID、英文名或中文名不同而被拆分统计的问题，同时统一队伍去重、Top5 队友、助战率、界域与类型筛选。':'Unified team-search and leaderboard statistics: the current season merges the base cache with the Top 500 overlay by UID and synchronizes the latest ranks. Character identities are normalized to SKeyDB canonical IDs, preventing one Awakener from being split by game ID, English name, or Chinese name; team deduplication, Top 5 teammates, assist rate, Realm, and role filters now use the same identity model.',
'2026-09-24 · 英文适配完善':'2026-09-24 · English Adaptation Completed','完成忘忘看报英文模式的系统性审阅与补全：覆盖旧版 v0.3 启动框架、伤害计算器角色专属资源、伤害事件明细、融灾榜单与旧版425榜单、爱的节奏榜、留言板、更新日志、关于页及动态生成文本；Waline 留言板语言会随中英文切换同步更新，并修复动态文本、title、placeholder、aria-label 在重绘后的翻译监听。':'Completed a systematic review and expansion of Morimens Weekly’s English mode, covering the legacy v0.3 bootstrap UI, character-specific calculator resources, damage-event details, D-Zone and legacy 425 leaderboards, Love Rhythm, Guestbook, Changelog, About page, and dynamically generated text. Waline now follows the selected language, and dynamic text plus title, placeholder, and aria-label changes are retranslated after rerendering.','2026-09-21 · 爱的节奏榜':'2026-09-21 · Love Rhythm Ranking',
'新增弥萨格校猫；增加“无恶意，纯节奏”免责声明；新增总分、爱数量、拉黑数量与总热度（爱 + 拉黑）四种排序方式，并保留 Waline 跨设备在线计数。':'Added the Misag school cat, the “no malice, just rhythm” disclaimer, and four sort modes: total score, likes, dislikes, and total heat (likes + dislikes), while retaining Waline cross-device online counts.',
'2026-09-20 · 留言板':'2026-09-20 · Guestbook',
'新增“留言板”标签，接入 Waline 在线留言服务，并按忘忘看报的深色玻璃质感与金色强调色进行样式适配。':'Added the Guestbook tab with Waline online comments, styled to match Morimens Weekly’s dark glass treatment and gold accents.',
'感谢':'Thanks to',
'提供融灾榜单与挑战记录；感谢':'for D-Zone leaderboard and challenge records; thanks to',
...
'400–409 分':'400–409 pts','390–399 分':'390–399 pts','380–389 分':'380–389 pts','370–379 分':'370–379 pts',
'360–369 分':'360–369 pts','350–359 分':'350–359 pts',
'当前为':'Current scope: ',
'，统计所有已下载用户，并包含暂时无法匹配榜单名次的用户。':'. All downloaded users are counted, including users whose leaderboard rank cannot currently be matched.',
'当前快照实际抓取到的最高榜单名次为':'The current snapshot reaches leaderboard rank',
'命轮出场率按“该期命轮总使用次数 ÷ 统计总人数”计算；统计总人数 = 队伍总数 ÷ 10。平均出场率按各期统计人数加权，首期无命轮数据，不参与统计。':'Wheel appearance rate = total Wheel uses in the period ÷ counted players; counted players = total teams ÷ 10. The average is weighted by each period’s counted population. The first period has no Wheel data and is excluded.',
'旧版融灾425出场率数据来源：':'Legacy D-Zone 425 data source: ',
'在线文档':'online document',
'出场率计算：使用次数 ÷ 统计人数；命轮按每人 10 队折算，使用次数 ÷（总队伍数 ÷ 10）计算。':'Appearance rate = uses ÷ counted players. Wheel rate assumes 10 teams per player: uses ÷ (total teams ÷ 10).',
'角色等级 / 内在灵格':'Character Level / Inner Spirit',
'多段技能倍率':'Multi-hit Skill Multiplier',
'技能条件开关':'Skill Condition Toggles',
'命轮正位阶数':'Wheel Refinement Tier',
'立绘 · 头像 · 台词每日签':'Artwork · Portrait · Voice-line Daily Fortune',
'① 角色、等级与技能':'① Awakener, Level & Skill',
'角色分类同步准备中':'Preparing Awakener-category sync',
'角色等级':'Awakener Level',
'等待角色数据':'Waiting for Awakener data',
'0 · 不计入':'0 · Disabled',
'使用维基角色等级 / 升格属性自动填入攻击与可识别百分比':'Use character level / progression data to auto-fill ATK and recognized percentages',
'倍率模式':'Coefficient Mode',
'首个直接伤害倍率（推荐）':'First Direct-Damage Coefficient (recommended)',
'合计识别倍率':'Combined Recognized Coefficient',
'重复命中次数':'Repeated Hit Count',
'明确的重复命中才填写，避免与合计倍率重复计算。':'Enter only explicit repeated hits to avoid double-counting the combined coefficient.',
'自动属性开启时按等级表更新；可关闭后手填。':'With auto stats enabled, values follow level scaling; disable it to enter them manually.',
'当前按主动伤害基础项 + 力量实验处理。':'Currently modeled as the Active-DMG base term plus STR.',
'选择角色后读取维基技能表。':'Select an Awakener to load the skill table.',
'等待技能倍率解析。':'Waiting for skill-coefficient parsing.',
'技能条件 / 叠层（默认不启用）':'Skill Conditions / Stacks (disabled by default)',
'选择技能后显示可识别条件。':'Recognized conditions appear after selecting a skill.',
'主属性 + 正位阶数':'Primary Stat + Refinement Tier',
'正位阶数':'Refinement Tier',
'正位 0 阶':'Refinement 0',
'正位 1 阶':'Refinement 1',
'正位 2 阶':'Refinement 2',
'正位 3 阶':'Refinement 3',
'正位 4 阶 / 叠位':'Refinement 4 / Stack',
'启用当前正位效果':'Enable Current Refinement Effect',
'主属性始终计入；条件型效果确认后才计入。':'Primary stats are always included; conditional effects are included only when confirmed.',
'未选择命轮':'No Wheel selected',
'当前阶位效果会显示在这里。':'The current refinement effect is shown here.',
'选择命轮后读取主属性与正位文本。':'Select a Wheel to load its primary stat and refinement text.',
'夜':'Night',
'等待角色资料':'Waiting for Awakener profile',
'角色资料':'Awakener Profile',
'角色台词':'Awakener Voice Line',
'等待同步。':'Waiting for sync.',
'今日宜':'Recommended Today',
'今日忌':'Avoid Today',
'换一句台词':'Another Voice Line',
