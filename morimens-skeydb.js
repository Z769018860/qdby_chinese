(()=>{
  ...
  function render(rec,random=false){
    ...
    const realmBadge=$('fortuneRealmBadge'),realmIcon=$('fortuneRealmIcon'),realmName=$('fortuneRealmName'),realmKey=Object.keys(realms).find(key=>String(rec.realm||'').toUpperCase()===key)||Object.keys(realms).find(key=>realms[key]===loc.realm);if(realmBadge&&realmIcon&&realmName&&realmKey){realmIcon.hidden=false;realmIcon.src=`assets/morimens/realms-svg/Icon_Career2_${realmIcons[realmKey]}.svg?v=${assetVersion()}`;realmIcon.alt=loc.realm;realmName.textContent=loc.realm;realmBadge.hidden=false;realmIcon.onerror=()=>{realmIcon.onerror=null;realmIcon.src=`assets/morimens/realms/Icon_Career2_${realmIcons[realmKey]}.webp?v=${assetVersion()}`}}else if(realmBadge)realmBadge.hidden=true;
