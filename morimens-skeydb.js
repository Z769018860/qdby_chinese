(()=>{
  ...
  async function downloadFortune(){
    if(!current)throw new Error(isZh()?'请先生成今日签':'Generate the Daily Fortune first');
    const en=!isZh(),canvas=document.createElement('canvas'),width=900,pad=54;canvas.width=width;canvas.height=1080;
    const ctx=canvas.getContext('2d');if(!ctx)throw new Error(en?'This browser does not support image export':'浏览器不支持图片导出');
    ...
    ctx.fillStyle='rgba(213,177,118,.12)';ctx.fillRect(0,0,width,8);ctx.fillStyle='#f1d69f';ctx.font='bold 30px system-ui,sans-serif';ctx.fillText(en?'Morimens Weekly · Daily Fortune':'忘忘看报 · 今日签',pad,65);
    ...
    ctx.fillStyle='#f5f8ff';ctx.font='bold 48px system-ui,sans-serif';ctx.fillText($('fortuneTitle')?.textContent||(en?'Today’s Awakener':'今日唤醒体'),pad,190);
    ctx.fillStyle='#e7cf9d';ctx.font='bold 24px system-ui,sans-serif';ctx.fillText($('fortuneLevel')?.textContent||(en?'Daily Fortune':'今日签'),pad,235);
    const lines=[$('fortuneText')?.textContent||'',$('fortuneQuote')?.innerText||'',(en?'Recommended: ':'今日宜：')+($('fortuneGood')?.textContent||'—'),(en?'Avoid: ':'今日忌：')+($('fortuneBad')?.textContent||'—'),(en?'Lucky Multiplier: ':'幸运乘区：')+($('fortuneStat')?.textContent||'—'),(en?'Keywords: ':'今日关键词：')+($('fortuneKeyword')?.textContent||'—')].filter(Boolean);let y=310;ctx.font='22px system-ui,sans-serif';for(const line of lines){const chars=String(line).split('');let buf='';for(const ch of chars){if(ctx.measureText(buf+ch).width>width-pad*2){ctx.fillText(buf,pad,y);y+=36;buf=''}buf+=ch}if(buf){ctx.fillText(buf,pad,y);y+=42}y+=10}
    ...
    const blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/png'));if(!blob)throw new Error(en?'Image generation failed':'图片生成失败');const url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=(en?'daily-fortune-':'今日签-')+todayKey()+'.png';document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),60000);
  }
  function ensureUi(){
    const actions=document.querySelector('.fortuneActions');if(actions&&!$('skeydbQuoteBtn')){const b=document.createElement('button');b.id='skeydbQuoteBtn';b.type='button';b.className='ghostBtn';b.textContent=isZh()?'换一句角色台词':'Another Voice Line';b.addEventListener('click',()=>{if(!current)return;quoteIndex++;renderQuote(current)});actions.insertBefore(b,actions.lastElementChild)}const download=$('downloadFortuneBtn');if(download&&!download.dataset.bound){download.dataset.bound='1';download.addEventListener('click',async()=>{download.disabled=true;download.textContent=isZh()?'正在生成…':'Generating…';try{await downloadFortune()}catch(error){console.error('Daily Fortune export failed',error);alert(error.message)}finally{download.disabled=false;download.textContent=isZh()?'下载今日签':'Download Fortune'}})}
    const tags=document.querySelector('.heroTags');if(tags&&!$('skeydbStatus')){const s=document.createElement('span');s.className='tag';s.id='skeydbStatus';s.textContent=isZh()?'SKeyDB / Wiki：等待同步快照':'SKeyDB / Wiki: waiting for synchronized snapshot';tags.appendChild(s)}
  ...
  function render(rec,random=false){
    ...
    const realmBadge=$('fortuneRealmBadge'),realmIcon=$('fortuneRealmIcon'),realmName=$('fortuneRealmName'),realmKey=Object.keys(realms).find(key=>String(rec.realm||'').toUpperCase()===key)||Object.keys(realms).find(key=>realms[key]===loc.realm);if(realmBadge&&realmIcon&&realmName&&realmKey){realmIcon.hidden=false;realmIcon.src=`assets/morimens/realms-svg/Icon_Career2_${realmIcons[realmKey]}.svg`;realmIcon.alt=isZh()?`${loc.realm}界域`:`${loc.realm} Realm`;realmName.textContent=loc.realm;realmBadge.hidden=false;realmIcon.onerror=()=>{realmIcon.hidden=true;let fallback=realmBadge.querySelector('.fortuneRealmFallback');if(!fallback){fallback=document.createElement('span');fallback.className='fortuneRealmFallback';realmBadge.insertBefore(fallback,realmName)}fallback.textContent=isZh()?(realmMarks[realmKey]||'域'):(String(realmKey||'R').slice(0,1));fallback.title=isZh()?`${loc.realm}界域`:`${loc.realm} Realm`}}else if(realmBadge)realmBadge.hidden=true;
    ...
    renderQuote(rec);
    const seed=hash(`${todayKey()}-${rec.id}`),keywords=[loc.realm,loc.type,wheelName,quotes[quoteIndex]?.title||(isZh()?'角色语音':'Voice line')].filter(Boolean).slice(0,4);if($('fortuneStat'))$('fortuneStat').textContent=`× ${(1+(seed%36)/100).toFixed(2)}`;if($('fortuneKeyword'))$('fortuneKeyword').textContent=keywords.join(' · ');
