(()=>{
  ...
  function injectStyle(){
    ...
    const s=document.createElement('style');s.id='morimensDtideStyle';s.textContent=`
      ...
      .dtideLegacyShare{margin:0 auto 10px 0;padding:7px 12px;border:1px solid #b99a61;border-radius:9px;background:rgba(185,154,97,.16);color:#f1d69f;font:700 12px/1.4 system-ui,sans-serif;cursor:pointer}.dtideLegacyShare:hover{background:rgba(185,154,97,.28)}.dtideLegacyShare:disabled{opacity:.6;cursor:wait}.dtideLegacyShare[hidden]{display:none}
...
function sortUsageRows(a,b,groups,waves){const spec=$('dtideSort')?.value||'total-desc';const m=spec.match(/^wave(\d+)-(asc|desc)$/);let av=a.total||a.count||0,bv=b.total||b.count||0;if(m){const w=Number(m[1]),g=groups?.get(w),find=x=>g?.characters?.find(y=>y.key===x.key)?.count||0;av=find(a);bv=find(b)}const d=bv-av;return spec.endsWith('-asc')?-d:d||String(a.name||a.key).localeCompare(String(b.name||b.key),'zh-CN')}
  function renderLegacyMatrix(){
    ...
    let share=$('dtideLegacyShare');if(!share){share=document.createElement('button');share.id='dtideLegacyShare';share.type='button';share.className='dtideLegacyShare';$('dtideMatrix')?.before(share)}
    share.hidden=false;share.textContent='分享表格图片';share.onclick=async()=>{
      share.disabled=true;share.textContent='正在生成图片…';
      try{
        const canvas=document.createElement('canvas'),ctx=canvas.getContext('2d');if(!ctx)throw new Error('浏览器不支持图片导出');
        const width=1920,pad=42,nameWidth=240,avgWidth=168,colWidth=(width-pad*2-nameWidth-avgWidth)/periods.length,rowHeight=46,headerY=138,headerHeight=74,footerHeight=100;
        canvas.width=width;canvas.height=headerY+headerHeight+names.length*rowHeight+footerHeight;
        ctx.fillStyle='#0e1624';ctx.fillRect(0,0,width,canvas.height);
        ctx.fillStyle='#f1d69f';ctx.font='bold 30px system-ui,sans-serif';ctx.fillText('旧版融灾高难出场率',pad,58);
        ctx.fillStyle='#aebbd0';ctx.font='19px system-ui,sans-serif';ctx.fillText(`来源：@却尘  ·  ${names.length} 位角色  ·  ${periods.length} 期`,pad,96);
        ctx.fillStyle='#192638';ctx.fillRect(pad,headerY,width-pad*2,headerHeight);
        ctx.textBaseline='middle';ctx.textAlign='left';ctx.fillStyle='#f2f5fb';ctx.font='bold 20px system-ui,sans-serif';ctx.fillText('角色',pad+12,headerY+headerHeight/2);
        ctx.textAlign='center';
        periods.forEach((p,i)=>{const x=pad+nameWidth+(i+.5)*colWidth;ctx.fillStyle='#f2f5fb';ctx.font='bold 17px system-ui,sans-serif';ctx.fillText(p.date,x,headerY+25);ctx.fillStyle=realmColor[p.realm]||'#dce5ef';ctx.font='bold 17px system-ui,sans-serif';ctx.fillText(`【${p.realm}】`,x,headerY+52)});
        ctx.fillStyle='#f2f5fb';ctx.font='bold 17px system-ui,sans-serif';ctx.fillText('平均出场率',width-pad-avgWidth/2,headerY+headerHeight/2);
        const avatarUrls=names.map(name=>characterInfo(name,{name,skeydbId:season.characterMap?.[name]?.skeydbId,image:season.characterMap?.[name]?.image}).image);
        const avatars=await Promise.all(avatarUrls.map(async src=>{if(!src)return null;try{const img=new Image();img.src=src;await img.decode();return img}catch{return null}}));
        names.forEach((name,index)=>{
          const y=headerY+headerHeight+index*rowHeight,values=periods.map(p=>cell(name,p.wave));
          ctx.fillStyle=index%2?'#152031':'#111b2a';ctx.fillRect(pad,y,width-pad*2,rowHeight);
          const avatar=avatars[index];if(avatar){ctx.save();ctx.beginPath();ctx.roundRect(pad+9,y+7,32,32,6);ctx.clip();ctx.drawImage(avatar,pad+9,y+7,32,32);ctx.restore()}
          ctx.textAlign='left';ctx.fillStyle='#e5edf8';ctx.font='bold 17px system-ui,sans-serif';ctx.fillText(name,pad+(avatar?49:12),y+rowHeight/2,nameWidth-(avatar?58:20));
          ctx.textAlign='center';ctx.font='bold 16px system-ui,sans-serif';
          values.forEach((v,i)=>{const t=Math.max(0,Math.min(1,v/Math.max(max,Number.EPSILON)));ctx.fillStyle=`hsla(${Math.round(215-215*t)},78%,46%,${(.52*t).toFixed(2)})`;ctx.fillRect(pad+nameWidth+i*colWidth,y,colWidth,rowHeight);ctx.fillStyle='#fff';ctx.fillText(rate(v),pad+nameWidth+(i+.5)*colWidth,y+rowHeight/2)});
          ctx.fillStyle='#f1d69f';ctx.fillText(rate(values.reduce((sum,v)=>sum+v,0)/(values.length||1)),width-pad-avgWidth/2,y+rowHeight/2);
          ctx.fillStyle='rgba(148,163,184,.13)';ctx.fillRect(pad,y+rowHeight-1,width-pad*2,1);
        });
        ctx.textAlign='right';ctx.fillStyle='#aebbd0';ctx.font='16px system-ui,sans-serif';ctx.fillText(`${location.origin}${location.pathname}`,width-pad,canvas.height-53);
        ctx.fillStyle='#f1d69f';ctx.font='bold 17px system-ui,sans-serif';ctx.fillText('copyright@青灯不弈',width-pad,canvas.height-25);
        const blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/png'));if(!blob)throw new Error('图片生成失败');
        const file=new File([blob],'旧版融灾高难出场率.png',{type:'image/png'});
        if(navigator.canShare?.({files:[file]})){try{await navigator.share({files:[file],title:'旧版融灾高难出场率'});return}catch(error){if(error?.name==='AbortError')return}}
      ...
      }catch(error){console.error('分享表格图片失败',error);alert(`图片生成失败：${error.message}`)}finally{share.disabled=false;share.textContent='分享表格图片'}
    };
    host.onclick=e=>{
  ...
  function renderMatrix(){
    ...
    const share=$('dtideLegacyShare');if(share)share.hidden=true;
