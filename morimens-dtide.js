...
function sortUsageRows(a,b,groups,waves){const spec=$('dtideSort')?.value||'total-desc';const m=spec.match(/^wave(\d+)-(asc|desc)$/);let av=a.total||a.count||0,bv=b.total||b.count||0;if(m){const w=Number(m[1]),g=groups?.get(w),find=x=>g?.characters?.find(y=>y.key===x.key)?.count||0;av=find(a);bv=find(b)}const d=bv-av;return spec.endsWith('-asc')?-d:d||String(a.name||a.key).localeCompare(String(b.name||b.key),'zh-CN')}
  function renderMatrix(){
    ...
    if(entity==='creation'){
      const creationGroup=rows=>{const map=new Map();for(const {team} of rows){const seen=new Set();for(const item of team.creations||[]){const key=String(item.id??item.name??'');if(!key||seen.has(key))continue;seen.add(key);const old=map.get(key)||{key,name:item.name||key,image:item.image||'',count:0};old.count++;if(!old.image&&item.image)old.image=item.image;map.set(key,old)}}const denom=rows.length||1;return [...map.values()].map(x=>({...x,teamRatePct:x.count/denom*100}))},waveGroups=new Map(waves.map(w=>[w,creationGroup(all.filter(x=>Number(x.wave.wave)===w))])),union=new Map();
