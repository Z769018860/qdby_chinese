(()=>{
  ...
  function characterInfo(key,fallback={}){
    ...
    return {name:displayCharacterName(identity?.name,loc?.name,fallback.canonicalName,fallback.name,rec?.name),image:localAsset(rec?.assets?.portrait||fallback.image||'','portrait'),art:rec?.assets?.card||localAsset(rec?.assets?.portrait||fallback.image||'','portrait'),id:id||key,ingameId:rec?.ingameId||fallback.ingameId};
  }
