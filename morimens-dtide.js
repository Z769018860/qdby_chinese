(()=>{
  ...
  function setupTabs(){
    ...
    const announcement=document.createElement('div');announcement.className='morimensAnnouncement';announcement.innerHTML='<strong>公告：</strong>目前只有融灾榜单在维护，其他标签为预留。因为数据更新不稳定，正在与 Eremora 作者沟通合作，所以暂时为 demo 版本。所有数据仅供娱乐，转载请说明来源。';
...
const dtideHeatStyle=(rate,max=35)=>{const safeMax=Math.max(Number(max)||0,Number.EPSILON),t=Math.max(0,Math.min(1,Number(rate||0)/safeMax)),h=Math.round(215-215*t),a=(.08+.34*t).toFixed(2);return `--dtide-heat:hsla(${h},78%,46%,${a})`};
function sortUsageRows(a,b,groups,waves){const spec=$('dtideSort')?.value||'total-desc';const m=spec.match(/^wave(\d+)-(asc|desc)$/);let av=a.total||a.count||0,bv=b.total||b.count||0;if(m){const w=Number(m[1]),g=groups?.get(w),find=x=>g?.characters?.find(y=>y.key===x.key)?.count||0;av=find(a);bv=find(b)}const d=bv-av;return spec.endsWith('-asc')?-d:d||String(a.name||a.key).localeCompare(String(b.name||b.key),'zh-CN')}
  ...
  function populateFilters(){
    ...
    const scoreValues=[...new Set(flattenTeams().map(({record})=>Number(record?.score)).filter(Number.isFinite))].sort((a,b)=>b-a);$('dtideTotalScore').innerHTML='<option value="all">全部分数</option>'+scoreValues.map(score=>`<option value="${score}">${score} 分</option>`).join('');const coverage=manifest.fieldCoverage||{};$('dtideProgression').disabled=!coverage.enlightenLevel;$('dtideWheel').disabled=!coverage.wheels;$('dtideCovenant').disabled=!coverage.covenants;
