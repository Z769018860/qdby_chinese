(()=>{
  ...
  const mediaBase='https://media.eremora.com/media/90a58fb3327e/icon/';
  ...
  function panelHtml(){return `
    ...
    <section class="panel">
      ...
      <div class="dtideFilters" style="margin-top:15px">
        ...
        <div class="dtideField span2"><label>界域</label><div class="dtideFilterPreset" id="dtideRealmFilters">${realmOrder.map(x=>`<button type="button" class="dtideFilterChip" data-realm="${x}"><img src="${mediaBase}${realmIcons[x]}" alt="">${realmZh[x]}</button>`).join('')}</div></div>
        <div class="dtideField span2"><label>定位</label><div class="dtideFilterPreset" id="dtideRoleFilters">${roleOrder.map(x=>`<button type="button" class="dtideFilterChip" data-role="${x}">${roleZh[x]}</button>`).join('')}</div></div>
  ...
  function computeGroup(rows){
    ...
    for(const {team} of rows){const seenC=new Set(),seenW=new Set(),seenS=new Set();for(const m of team.members||[]){memberSlots++;const ck=String(memberKey(m)),ec=enlightClass(m);countRate(enlight,ec,{name:enlightZh[ec]});if(!seenC.has(ck)){seenC.add(ck);countRate(chars,ck,{id:m.skeydbId||null,ingameId:m.ingameId||null,name:characterInfo(ck,m).name,image:m.image||null,borrowedCount:0});const cv=chars.get(ck);if(m.borrowed)cv.borrowedCount=(cv.borrowedCount||0)+1}let bc=byChar.get(ck);if(!bc){bc={key:ck,id:m.skeydbId||null,ingameId:m.ingameId||null,name:characterInfo(ck,m).name,image:m.image||null,appearances:0,levels:[],enlight:new Map(),wheels:new Map(),covs:new Map()};byChar.set(ck,bc)}bc.appearances++;if(m.level!=null)bc.levels.push(Number(m.level));countRate(bc.enlight,ec,{name:enlightZh[ec]});for(const w of m.wheels||[]){wheelSlots++;const wk=w.id??w.name;if(wk==null)continue;const wheelKey=String(wk);if(!seenW.has(wheelKey)){seenW.add(wheelKey);countRate(wheels,wheelKey,{id:w.id??null,name:wheelName(w),image:w.image||null,stacks:new Map()});const wheel=wheels.get(wheelKey),stackKey=wheelStackClass(w);countRate(wheel.stacks,stackKey,{name:wheelStackZh[stackKey]})}countRate(bc.wheels,wk,{id:w.id??null,name:wheelName(w),image:w.image||null})}for(const c of m.covenants||((m.covenant)?[m.covenant]:[])){covenantSlots++;const sk=c.id??c.name;if(sk==null)continue;if(!seenS.has(String(sk))){seenS.add(String(sk));countRate(covs,sk,{id:c.id??null,name:c.name||sk,image:c.image||null})}countRate(bc.covs,sk,{id:c.id??null,name:c.name||sk,image:c.image||null})}}}
...
function sortUsageRows(a,b,groups,waves){const spec=$('dtideSort')?.value||'total-desc';const m=spec.match(/^wave(\d+)-(asc|desc)$/);let av=a.total||a.count||0,bv=b.total||b.count||0;if(m){const w=Number(m[1]),g=groups?.get(w),find=x=>g?.characters?.find(y=>y.key===x.key)?.count||0;av=find(a);bv=find(b)}const d=bv-av;return spec.endsWith('-asc')?-d:d||String(a.name||a.key).localeCompare(String(b.name||b.key),'zh-CN')}
  function renderMatrix(){
    ...
    const all=scopedRows({rankCap:cap,difficulty,wave:'all',ct}),waves=[...new Set(all.map(x=>Number(x.wave.wave)).filter(Number.isFinite))].sort((a,b)=>a-b),groups=new Map(waves.map(w=>[w,computeGroup(all.filter(x=>Number(x.wave.wave)===w))]));
    ...
    if(entity==='wheel'){
      const union=new Map();for(const [,g] of groups)for(const wheel of g.wheels||[])union.set(wheel.key,wheel);
      ...
      host.innerHTML=`<table class="dtideTable"><thead><tr><th>命轮 / 叠位比例</th>${waves.map(w=>`<th><button type="button" class="dtideSortHead" data-sort-key="${w}" title="点击切换升降序">Wave ${w}${arrow(String(w))}</button></th>`).join('')}<th><button type="button" class="dtideSortHead" data-sort-key="total" title="点击切换升降序">总出现${arrow('total')}</button></th></tr></thead><tbody>${rows.map(wheel=>`<tr><td><div class="dtideChar">${wheel.image?`<img class="dtideGearIcon" src="${esc(wheel.image)}" alt="">`:''}<span>${esc(wheel.name||wheel.key)}</span>${ratioBar(wheel)}</div></td>${waves.map(w=>{const hit=(groups.get(w)?.wheels||[]).find(x=>x.key===wheel.key);return `<td class="dtideRate">${pct(hit?.teamRatePct||0)}</td>`}).join('')}<td>${wheel.total}</td></tr>`).join('')}</tbody></table>`;
  ...
  function boot(){injectStyle();setupTabs();document.addEventListener('click',e=>{const chip=e.target.closest('.dtideFilterChip');if(!chip)return;chip.classList.toggle('isActive');if(filtersReady)renderResults()});window.addEventListener('morimens-language-change',()=>{if($('morimensBuilderTab')){$('morimensBuilderTab').textContent=zh()?'伤害计算 / 每日签':'Damage / Fortune';$('morimensDtideTab').textContent=zh()?'融灾榜单':'D-Zone Leaderboard'}relocalizeControls()})}
