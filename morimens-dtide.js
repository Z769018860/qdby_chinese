(()=>{
  ...
  function injectStyle(){
    ...
    const s=document.createElement('style');s.id='morimensDtideStyle';s.textContent=`
      ...
      .dtideTable .dtideLegacyDetailRow>td{padding:0;border-bottom:1px solid rgba(148,163,184,.16)}.dtideLegacyDetail{background:linear-gradient(145deg,#142338,#0e1a2d);border-top:1px solid rgba(145,185,221,.18);color:#eaf2ff}.dtideLegacyDetail>strong{display:block;padding:13px 16px 0;font:700 13px/1.5 system-ui,sans-serif;letter-spacing:.02em;color:#f5f8ff}.dtideLegacyDetail svg{display:block;width:100%;height:270px;font-family:system-ui,sans-serif;font-variant-numeric:tabular-nums;overflow:visible}
...
function sortUsageRows(a,b,groups,waves){const spec=$('dtideSort')?.value||'total-desc';const m=spec.match(/^wave(\d+)-(asc|desc)$/);let av=a.total||a.count||0,bv=b.total||b.count||0;if(m){const w=Number(m[1]),g=groups?.get(w),find=x=>g?.characters?.find(y=>y.key===x.key)?.count||0;av=find(a);bv=find(b)}const d=bv-av;return spec.endsWith('-asc')?-d:d||String(a.name||a.key).localeCompare(String(b.name||b.key),'zh-CN')}
  function renderLegacyMatrix(){
    ...
    host.onclick=e=>{const button=e.target.closest('[data-legacy-sort]');if(button){const key=button.dataset.legacySort;if(window.__legacyMatrixSort===key)window.__legacyMatrixAscending=!window.__legacyMatrixAscending;else{window.__legacyMatrixSort=key;window.__legacyMatrixAscending=false}return renderLegacyMatrix()};const row=e.target.closest('[data-legacy-character]');if(!row)return;const old=row.nextElementSibling;if(old?.classList.contains('dtideLegacyDetailRow'))return old.remove();const detail=document.createElement('tr');detail.className='dtideLegacyDetailRow';detail.innerHTML=`<td colspan="${periods.length+2}"><div class="dtideLegacyDetail"><strong>${esc(row.dataset.legacyCharacter)} · 各期出场率排名变化</strong><svg role="img" aria-label="${esc(row.dataset.legacyCharacter)}各期排名折线图"></svg></div></td>`;row.after(detail);drawChart(detail,row.dataset.legacyCharacter,row.closest('table'))};
