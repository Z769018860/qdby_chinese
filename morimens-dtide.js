...
function sortUsageRows(a,b,groups,waves){const spec=$('dtideSort')?.value||'total-desc';const m=spec.match(/^wave(\d+)-(asc|desc)$/);let av=a.total||a.count||0,bv=b.total||b.count||0;if(m){const w=Number(m[1]),g=groups?.get(w),find=x=>g?.characters?.find(y=>y.key===x.key)?.count||0;av=find(a);bv=find(b)}const d=bv-av;return spec.endsWith('-asc')?-d:d||String(a.name||a.key).localeCompare(String(b.name||b.key),'zh-CN')}
  ...
  function legacyDetailExtras(name,periods){
    ...
    return '<div class="dtideLegacyExtra" style="position:relative;z-index:1;display:grid;grid-template-columns:minmax(0,1.5fr) minmax(260px,1fr);gap:12px;margin:12px 12px 16px;text-align:left"><section class="dtideInsightPanel" style="min-width:0;background:rgba(12,20,33,.94)"><h4>常用配队 Top 10 <small>完整四人队</small></h4><div class="dtideSquadList">'+squads+'</div></section><section class="dtideInsightPanel" style="min-width:0;background:rgba(12,20,33,.94)"><h4>每期助战使用率</h4><div class="dtideAssistRows">'+assistHtml+'</div></section><section class="dtideInsightPanel dtideAssistRankPanel"><h4>助战使用率排名折线图 <small>名次越靠上越高</small></h4><svg class="dtideAssistRankChart" role="img" aria-label="'+esc(name)+'各期助战使用率排名折线图"></svg></section></div>';
  }
  function drawLegacyAssistRankChart(detail,name,periods){
    const svg=detail?.querySelector('.dtideAssistRankChart');if(!svg)return;
    const characters=legacyStructured?.characters||{},characterNames=Object.keys(characters),realmColor={混沌:'#e7b65c',超维:'#b98cff',深海:'#5cb8ff',血肉:'#ff7180'};
    const own=characters[name]?.assistRates||{},rateFor=(char,period)=>Number(characters[char]?.assistRates?.[period.label]||0);
    const points=periods.map(period=>{
      if(Number(period.teamCount||0)<=0)return {period,rate:Number(own[period.label]||0),rank:null};
      const rate=Number(own[period.label]||0),rank=1+characterNames.reduce((count,char)=>count+(rateFor(char,period)>rate+1e-12?1:0),0);
      return {period,rate,rank};
    });
    const bounds=svg.getBoundingClientRect(),width=Math.max(720,bounds.width||0),height=260,left=48,right=22,top=28,bottom=205,plotWidth=width-left-right,maxRank=Math.max(1,characterNames.length);
    const x=i=>left+(periods.length<=1?plotWidth/2:i*plotWidth/(periods.length-1)),y=rank=>top+(Math.max(1,rank)-1)*(bottom-top)/Math.max(1,maxRank-1);
    const grid=[1,10,20,30,40,50,maxRank].filter((v,i,a)=>v<=maxRank&&a.indexOf(v)===i).sort((a,b)=>a-b);
    const segments=[];for(let i=1;i<points.length;i++)if(points[i-1].rank!=null&&points[i].rank!=null)segments.push('<line x1="'+x(i-1)+'" y1="'+y(points[i-1].rank)+'" x2="'+x(i)+'" y2="'+y(points[i].rank)+'" stroke="#69b8ff" stroke-width="2.5" stroke-linecap="round"/>');
    svg.setAttribute('viewBox','0 0 '+width+' '+height);
    svg.innerHTML=grid.map(rank=>'<line x1="'+left+'" y1="'+y(rank)+'" x2="'+(width-right)+'" y2="'+y(rank)+'" stroke="rgba(166,193,224,.17)" stroke-dasharray="4 5"/><text x="'+(left-9)+'" y="'+(y(rank)+4)+'" fill="#aebfd3" font-size="11" text-anchor="end">'+rank+'</text>').join('')+
      periods.map((period,i)=>'<line x1="'+x(i)+'" y1="'+top+'" x2="'+x(i)+'" y2="'+bottom+'" stroke="rgba(166,193,224,.08)"/>').join('')+
      '<line x1="'+left+'" y1="'+top+'" x2="'+left+'" y2="'+bottom+'" stroke="#7893b2" stroke-width="1.2"/>'+segments.join('')+
      points.map((point,i)=>point.rank==null?'<text x="'+x(i)+'" y="'+(bottom-6)+'" fill="#718096" font-size="10" text-anchor="middle">无排名</text>':'<circle cx="'+x(i)+'" cy="'+y(point.rank)+'" r="5.5" fill="'+(realmColor[point.period.realm]||'#69b8ff')+'" stroke="#eef7ff" stroke-width="1.5"><title>'+esc(point.period.date)+'【'+esc(point.period.realm)+'】：第 '+point.rank+' 名 · '+(point.rate*100).toFixed(2)+'%</title></circle><text x="'+x(i)+'" y="'+Math.max(16,y(point.rank)-11)+'" fill="#f2f7ff" font-size="11" font-weight="800" text-anchor="middle">'+point.rank+'</text>').join('')+
      periods.map((period,i)=>'<text x="'+x(i)+'" y="'+(height-30)+'" fill="#d7e2ef" font-size="10.5" text-anchor="middle">'+esc(period.date)+'</text><text x="'+x(i)+'" y="'+(height-12)+'" fill="'+(realmColor[period.realm]||'#aebfd3')+'" font-size="10.5" font-weight="800" text-anchor="middle">【'+esc(period.realm)+'】</text>').join('');
  }
  ...
  function renderLegacyMatrix(){
    ...
    host.onclick=e=>{
      ...
      detail.innerHTML=`<td colspan="${periods.length+2}"><div class="dtideLegacyDetail"><strong>${esc(name)} · 各期出场率排名变化</strong><svg role="img" aria-label="${esc(name)}各期排名折线图"></svg>`+legacyDetailExtras(name,periods)+`</div></td>`;
      row.after(detail);
      ...
      drawLegacyAssistRankChart(detail,name,periods);
    };
    if(host.__legacyResizeHandler)window.removeEventListener('resize',host.__legacyResizeHandler);
    host.__legacyResizeHandler=()=>{const table=host.querySelector('table');if(!table)return;for(const detail of table.querySelectorAll('.dtideLegacyDetailRow')){const name=detail.previousElementSibling?.dataset.legacyCharacter;drawChart(detail,name,table);drawLegacyAssistRankChart(detail,name,periods)}};
