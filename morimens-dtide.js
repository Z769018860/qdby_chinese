(()=>{
  ...
  const enlightOrder=['e0','e1','e2','e3','oe','aa'];
  const enlightZh={e0:'E0',e1:'E1',e2:'E2',e3:'E3',oe:'OE',aa:'AA',unknown:'未知'};
  const enlightColors={e0:'#8893a4',e1:'#67a9cf',e2:'#62c4a8',e3:'#d9a441',oe:'#62b7ff',aa:'#d978d0',unknown:'#6b7280'};
  ...
  function injectStyle(){
    ...
    const s=document.createElement('style');s.id='morimensDtideStyle';s.textContent=`
      ...
      .dtideUsageCards{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.dtideUsage{padding:10px;border:1px solid rgba(148,163,184,.12);border-radius:11px;background:rgba(255,255,255,.028);display:flex;justify-content:space-between;gap:10px;align-items:center}.dtideUsage b{font-size:12px}.dtideUsage small{display:block;color:#77869a;margin-top:3px}.dtideUsage strong{font-size:15px;color:#e9d0a0;white-space:nowrap}.dtideEnlightItem{border-left:4px solid var(--dtide-enlight-color)!important}.dtideEnlightItem b,.dtideEnlightItem strong{color:var(--dtide-enlight-color)!important}.dtideMemberAvatar{width:30px;height:30px;border-radius:8px;object-fit:cover;float:left;margin:0 7px 3px 0;background:#0b1220}
  ...
  function enlightClass(m){const ms=String(m?.enlightenMilestone||m?.enlightTier||m?.progression||'').toUpperCase();if(['E0','E1','E2','E3','OE','AA'].includes(ms))return ms.toLowerCase();if(ms==='LAW12')return 'aa';if(ms==='OVERLIMIT')return 'oe';return 'unknown'}
...
function sortUsageRows(a,b,groups,waves){const spec=$('dtideSort')?.value||'total-desc';const m=spec.match(/^wave(\d+)-(asc|desc)$/);let av=a.total||a.count||0,bv=b.total||b.count||0;if(m){const w=Number(m[1]),g=groups?.get(w),find=x=>g?.characters?.find(y=>y.key===x.key)?.count||0;av=find(a);bv=find(b)}const d=bv-av;return spec.endsWith('-asc')?-d:d||String(a.name||a.key).localeCompare(String(b.name||b.key),'zh-CN')}
  ...
  function renderEquipment(){
    ...
    const section=(title,arr,rateKey='teamRatePct',denom='支队伍',isEnlight=false)=>`<h4 style="margin:10px 0 7px;font-size:12px">${title}</h4><div class="dtideUsageCards">${(arr||[]).slice(0,24).map(x=>{const group=x.key||x.id||'unknown',color=enlightColors[group]||enlightColors.unknown;return `<div class="dtideUsage${isEnlight?' dtideEnlightItem':''}"${isEnlight?` style="--dtide-enlight-color:${color}"`:''}><div><b>${esc(isEnlight?(enlightZh[group]||x.name||group):(x.name||x.key))}</b><small>${x.count} ${denom}</small></div><strong>${pct(x[rateKey])}</strong></div>`}).join('')||'<div class="dtideEmpty">暂无</div>'}</div>`;
