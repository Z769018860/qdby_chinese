(()=>{
  ...
  function legacyRealmLabel(value){
    const raw=String(value||'');
    if(zh())return raw;
    return ({'混沌':'Chaos','深海':'Aequor','血肉':'Caro','超维':'Ultra',Chaos:'Chaos',Aequor:'Aequor',Caro:'Caro',Ultra:'Ultra'})[raw]||raw;
  }
...
function sortUsageRows(a,b,groups,waves){const spec=$('dtideSort')?.value||'total-desc';const m=spec.match(/^wave(\d+)-(asc|desc)$/);let av=a.total||a.count||0,bv=b.total||b.count||0;if(m){const w=Number(m[1]),g=groups?.get(w),find=x=>g?.characters?.find(y=>y.key===x.key)?.count||0;av=find(a);bv=find(b)}const d=bv-av;return spec.endsWith('-asc')?-d:d||String(a.name||a.key).localeCompare(String(b.name||b.key),'zh-CN')}
  ...
  function legacyDetailExtras(name,periods){
    const data=legacyStructured?.characters?.[name]||{},assist=data.assistRates||{},teamTotal=Number(data.teamTotal||0),realmColor={混沌:'#e7b65c',超维:'#b98cff',深海:'#5cb8ff',血肉:'#ff7180'},teamMap=new Map();
    for(const team of data.topTeams||[]){
      if(!Array.isArray(team.characters)||team.characters.length!==4)continue;
      const characters=[...team.characters].sort((a,b)=>String(a).localeCompare(String(b),'zh-CN')),key=characters.join('|'),row=teamMap.get(key)||{characters,count:0,periods:new Set()};
      row.count+=Number(team.count||0);if(team.period)row.periods.add(String(team.period));teamMap.set(key,row);
    }
    const teams=[...teamMap.values()].sort((a,b)=>b.count-a.count||a.characters.join('/').localeCompare(b.characters.join('/'),'zh-CN')).slice(0,10);
    const squads=teams.map((team,index)=>{
      ...
      const targetIndex=members.findIndex(member=>member.role===name);if(targetIndex>0)members.unshift(...members.splice(targetIndex,1));
    ...
    }).join('')||'<div class="dtideEmpty">'+ui('暂无配队数据','No team-composition data')+'</div>';
    const assistPeriods=periods.filter(p=>Number(p.teamCount||0)>0),assistHtml=assistPeriods.map(p=>'<div style="display:flex;justify-content:space-between;gap:12px;padding:6px 8px;border-bottom:1px solid rgba(166,193,224,.1)"><span>'+esc(p.date)+' <b class="dtideAssistRealm" style="color:'+(realmColor[p.realm]||'#dbe8f7')+'">【'+esc(legacyRealmLabel(p.realm))+'】</b></span><strong>'+((Number(assist[p.label]||0)*100).toFixed(2))+'%</strong></div>').join('');
    return '<div class="dtideLegacyExtra" style="position:relative;z-index:1;display:grid;grid-template-columns:minmax(0,1.5fr) minmax(260px,1fr);gap:12px;margin:12px 12px 16px;text-align:left"><section class="dtideInsightPanel" style="min-width:0;background:rgba(12,20,33,.94)"><h4>'+ui('常用配队 Top 10','Top 10 Team Compositions')+' <small>'+ui('完整四人队','complete four-member teams')+'</small></h4><div class="dtideSquadList">'+squads+'</div></section><section class="dtideInsightPanel" style="min-width:0;background:rgba(12,20,33,.94)"><h4>'+ui('每期助战使用率','Assist Usage Rate by Period')+'</h4><div class="dtideAssistRows">'+assistHtml+'</div></section></div>';
  }
  ...
  function enlightenmentDetailBlock(arr){
    const items=[...(arr||[])].sort((a,b)=>enlightOrder.indexOf(a.key)-enlightOrder.indexOf(b.key)),labels=zh()?enlightZh:enlightEn;
    return '<h4>'+ui('详细启灵比例','Detailed Enlighten Distribution')+'</h4><div class="dtideUsageCards">'+
