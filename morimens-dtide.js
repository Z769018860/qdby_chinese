(()=>{
  ...
  function setupTabs(){
    ...
    const tabs=document.createElement('nav');tabs.id='morimensTabs';tabs.className='morimensTabs';tabs.setAttribute('role','tablist');tabs.innerHTML='<button class="morimensTab" id="morimensDtideTab" role="tab" aria-selected="true" aria-controls="morimensDtidePanel">融灾榜单</button><button class="morimensTab" id="morimensCalcTab" role="tab" aria-selected="false" aria-controls="morimensCalcPanel">伤害计算器</button><button class="morimensTab" id="morimensFortuneTab" role="tab" aria-selected="false" aria-controls="morimensFortunePanel">每日签</button><button class="morimensTab" id="morimensChangelogTab" role="tab" aria-selected="false" aria-controls="morimensChangelogPanel">更新日志</button><button class="morimensTab" id="morimensCommentsTab" role="tab" aria-selected="false" aria-controls="morimensCommentsPanel">留言板</button><button class="morimensTab" id="morimensAboutTab" role="tab" aria-selected="false" aria-controls="morimensAboutPanel">关于</button>';
...
function sortUsageRows(a,b,groups,waves){const spec=$('dtideSort')?.value||'total-desc';const m=spec.match(/^wave(\d+)-(asc|desc)$/);let av=a.total||a.count||0,bv=b.total||b.count||0;if(m){const w=Number(m[1]),g=groups?.get(w),find=x=>g?.characters?.find(y=>y.key===x.key)?.count||0;av=find(a);bv=find(b)}const d=bv-av;return spec.endsWith('-asc')?-d:d||String(a.name||a.key).localeCompare(String(b.name||b.key),'zh-CN')}
  ...
  function legacyDetailExtras(name,periods){
    ...
    const squads=teams.map((team,index)=>{
      ...
      return '<div class="dtideSquadRow"><span class="dtideSquadRank">'+(index+1)+'</span><div class="dtideSquadMembers">'+members.map(member=>'<div class="dtideSquadMember">'+(member.image?'<img src="'+esc(member.image)+'" alt="" loading="lazy">':'')+'<span title="'+esc(member.name)+'">'+esc(member.name)+'</span></div>').join('')+'</div><div class="dtideSquadRate" title="'+esc(Number(team.count||0)+' / '+(denominator||0)+' 支含该角色队伍')+'"><b>'+(rate!=null?rate.toFixed(1)+'%':'—')+'</b><small>'+Number(team.count||0)+' 次</small></div></div>';
  ...
  function renderLegacyWheelMatrix(){
    ...
    if($('dtideStatus'))$('dtideStatus').textContent='旧版融灾425出场率（来源：@却尘）';
  ...
  function renderLegacyMatrix(){
    ...
    if(entity==='creation'){
      host.innerHTML='<div class="dtideEmpty">旧版融灾425出场率暂无造物榜单数据。</div>';
      host.onclick=null;
      ...
      if($('dtideStatus'))$('dtideStatus').textContent='旧版融灾425出场率（来源：@却尘）';
      if($('dtideCoverageWarn'))$('dtideCoverageWarn').innerHTML='<div class="dtideNotice">旧版融灾425出场率没有可用的造物榜单数据，因此该榜单保持为空。</div>';
      return true;
    }
    ...
    host.onclick=e=>{
      ...
      detail.innerHTML=`<td colspan="${periods.length+3}"><div class="dtideLegacyDetail"><strong>${esc(name)} · 各期出场率排名变化</strong><svg role="img" aria-label="${esc(name)}各期排名折线图"></svg>`+legacyDetailExtras(name,periods)+`</div></td>`;
      row.after(detail);
    ...
    if($('dtideCoverageWarn'))$('dtideCoverageWarn').innerHTML=`<div class="dtideNotice">旧版融灾425出场率数据来源：<a href="${esc(season.sourceUrl)}" target="_blank" rel="noopener noreferrer">在线文档</a>。<br><small>出场率计算：使用次数 ÷ 统计人数；命轮按每人 10 队折算，使用次数 ÷（总队伍数 ÷ 10）计算。</small></div>`;
    return true;
  }
