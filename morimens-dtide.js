...
function sortUsageRows(a,b,groups,waves){const spec=$('dtideSort')?.value||'total-desc';const m=spec.match(/^wave(\d+)-(asc|desc)$/);let av=a.total||a.count||0,bv=b.total||b.count||0;if(m){const w=Number(m[1]),g=groups?.get(w),find=x=>g?.characters?.find(y=>y.key===x.key)?.count||0;av=find(a);bv=find(b)}const d=bv-av;return spec.endsWith('-asc')?-d:d||String(a.name||a.key).localeCompare(String(b.name||b.key),'zh-CN')}
  ...
  function legacyDetailExtras(name,periods){
    ...
    const teamHtml=teams.length?teams.map((team,index)=>'<div style="display:grid;grid-template-columns:34px 1fr auto;gap:8px;align-items:center;padding:7px 10px;border-bottom:1px solid rgba(166,193,224,.12)"><b style="color:#e7b65c">#'+(index+1)+'</b><span>'+team.characters.map((role)=>{const meta=characterInfo(role,{name:role,skeydbId:season.characterMap?.[role]?.skeydbId,ingameId:season.characterMap?.[role]?.ingameId,image:season.characterMap?.[role]?.image});return '<span style="display:inline-flex;align-items:center;gap:5px;margin-right:6px"><span class="dtideChar" style="width:24px;height:24px">'+(meta.image?'<img src="'+esc(meta.image)+'" alt="" loading="lazy">':'')+'</span>'+esc(meta.name||role)+'</span>'}).join('')+'</span><small style="color:#b6c9df">'+Number(team.count||0)+' 次</small></div>').join(''):'<div style="padding:10px;color:#9fb0c4">暂无配队数据</div>';
