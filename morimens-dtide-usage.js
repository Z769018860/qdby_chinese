(()=>{
  ...
  function renderMatrix(){
    ...
    host.innerHTML=`<table class="dtideTable"><thead><tr><th><div class="dtideEntityHead"><span>${label}</span>${legend}</div></th>${waves.map(w=>`<th><button type="button" class="dtideSortHead" data-sort-key="${w}">Wave ${w}${arrow(String(w))}</button></th>`).join('')}${entity==='character'?`<th><button type="button" class="dtideSortHead" data-sort-key="assist">助战使用率${arrow('assist')}</button></th>`:''}<th><button type="button" class="dtideSortHead" data-sort-key="total">总出现${arrow('total')}</button></th></tr></thead><tbody>${rows.map((c,i)=>{const rank=c.fullRank||i+1;return `<tr${entity==='wheel'?` data-matrix-wheel-row="${esc(c.key)}"`:''}><td><div class="dtideRankedItem"><span class="dtideRankMark"><b>${rank}</b>${rankChange(rank,previousRanks.get(c.key))}</span>${entity==='character'?`<button type="button" class="dtideMatrixCharacter" data-matrix-character="${esc(c.key)}" aria-expanded="false"><span class="dtideChar">${c.image?`<img src="${esc(c.image)}" alt="">`:''}<span>${esc(c.name)}</span></span></button>${enlightBar(c)}`:`<a class="dtideChar dtideWikiLink" href="${esc(wikiUrl(c.name))}" target="_blank" rel="noopener noreferrer" title="在忘却前夜 Wiki 查看${esc(c.name)}">${c.image?`<img class="dtideGearIcon" src="${esc(c.image)}" data-fallback="${esc(c.fallbackImage||'')}" referrerpolicy="no-referrer" onerror="if(this.dataset.fallback&&this.src!==this.dataset.fallback){this.src=this.dataset.fallback;this.dataset.fallback=''}else{this.hidden=true}" alt="">`:''}<span>${esc(c.name)}</span></a>${entity==='wheel'?wheelRatioBar(c):''}`}</div></td>${waves.map(w=>{const hit=getHit(w,c.key),rate=hit?.teamRatePct||0;return `<td class="dtideRate dtideHeat" style="${heatStyle(rate)}">${pct(rate)}</td>`}).join('')}${entity==='character'?`<td class="dtideRate dtideHeat" style="${heatStyle(c.assistRatePct||0,100)}">${pct(c.assistRatePct||0)}</td>`:''}<td>${c.total}</td></tr>`}).join('')}</tbody></table>`;
