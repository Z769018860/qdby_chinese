(()=>{
  ...
  function activeAwakenerMembers(team){const realms=[...document.querySelectorAll('#dtideRealmFilters .dtideFilterChip.isActive')].map(x=>String(x.dataset.realm)),roles=[...document.querySelectorAll('#dtideRoleFilters .dtideFilterChip.isActive')].map(x=>String(x.dataset.role));return (team?.members||[]).filter(m=>(!realms.length||realms.includes(String(m.realm)))&&(!roles.length||roles.includes(String(m.role))))}
  function applyAwakenerFilters(rows){return rows.map(row=>{const members=activeAwakenerMembers(row.team);return members.length===(row.team.members||[]).length?row:{...row,team:{...row.team,members}}}).filter(row=>(row.team.members||[]).length)}
  ...
  function scopedRows({cap=selectedRankCap(),difficulty=$('dtideDifficulty')?.value||'all',wave='all',clearType=$('dtideClearType')?.value||'all'}={}){
    return applyAwakenerFilters(flatten().filter(x=>{
  ...
  function filteredDetailRows(){
    ...
    return applyAwakenerFilters(flatten(detailUsage?.records||[]).filter(x=>{if(!rankMatches(x.record,cap))return false;if(difficulty!=='all'&&x.difficulty!==difficulty)return false;if(!scoreMatches(x.record))return false;return true}));
  }
  ...
  function rowsFor(records,{cap,difficulty,clearType}){
    return applyAwakenerFilters(flatten(records).filter(x=>{if(!rankMatches(x.record,cap))return false;if(difficulty!=='all'&&x.difficulty!==difficulty)return false;if(!scoreMatches(x.record))return false;if(clearType!=='all'&&x.team.clearType!==clearType)return false;return true}));
  }
