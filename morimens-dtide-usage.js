(()=>{
  ...
  function characterInsightHtml(d){
    ...
    return `<div class="dtideCharacterInsight"><div class="dtideCharacterPortrait"><a class="dtideCharacterPortraitImage dtideWikiLink" href="${esc(p.wikiUrl||wikiUrl(p.name))}" target="_blank" rel="noopener noreferrer" title="在忘却前夜 Wiki 查看${esc(p.name||'角色')}">${p.art?`<img src="${esc(p.art)}" alt="${esc(p.name||'角色立绘')}" loading="lazy">`:'<span>暂无立绘</span>'}</a><b>${esc(p.name||'角色')}</b><small>点击立绘查看 Wiki</small></div><div class="dtideCharacterInsightRight"><section class="dtideInsightPanel"><h4>启灵分布</h4><div class="dtideInsightEnlight">${enlight}</div></section><section class="dtideInsightPanel"><h4>常用配队 Top 5 <small>完整四人队</small></h4><div class="dtideSquadList">${squads}</div></section></div></div>`;
  }
  ...
  function wheelInsightHtml(d){
    const users=(d.users||[]).map((x,index)=>'<div class="dtideSquadRow"><span class="dtideSquadRank">'+(index+1)+'</span><div class="dtideChar">'+(x.image?'<img src="'+esc(x.image)+'" alt="" loading="lazy">':'')+'<span><b>'+esc(x.name)+'</b></span></div><div class="dtideSquadRate"><b>'+pct(x.ratePct)+'</b><small>'+x.count+' 次使用</small></div></div>').join('')||'<div class="dtideEmpty">当前详细样本暂无使用角色记录</div>';
    const partners=(d.partners||[]).map((x,index)=>'<div class="dtideSquadRow"><span class="dtideSquadRank">'+(index+1)+'</span><div class="dtideChar">'+(x.image?'<img class="dtideGearIcon" src="'+esc(x.image)+'" data-fallback="'+esc(x.fallbackImage||'')+'" referrerpolicy="no-referrer" loading="lazy" onerror="if(this.dataset.fallback&&this.src!==this.dataset.fallback){this.src=this.dataset.fallback;this.dataset.fallback=\'\'}else{this.hidden=true}" alt="">':'')+'<span><b>'+esc(x.name)+'</b></span></div><div class="dtideSquadRate"><b>'+pct(x.ratePct)+'</b><small>'+x.count+' 次同装</small></div></div>').join('')||'<div class="dtideEmpty">当前样本暂无主副手搭配命轮记录</div>';
    return '<div class="dtideCharacterInsight"><div class="dtideCharacterPortrait"><a class="dtideCharacterPortraitImage dtideWikiLink" href="'+esc(wikiUrl(d.name))+'" target="_blank" rel="noopener noreferrer" title="在忘却前夜 Wiki 查看'+esc(d.name)+'">'+(d.image?'<img src="'+esc(d.image)+'" data-fallback="'+esc(d.fallbackImage||'')+'" referrerpolicy="no-referrer" loading="lazy" onerror="if(this.dataset.fallback&&this.src!==this.dataset.fallback){this.src=this.dataset.fallback;this.dataset.fallback=\'\'}else{this.hidden=true}" alt="'+esc(d.name)+'">':'<span>暂无图片</span>')+'</a><b>'+esc(d.name)+'</b><small>点击命轮图片查看 Wiki</small></div><div class="dtideCharacterInsightRight"><section class="dtideInsightPanel"><h4>Top 5 使用唤醒体 <small>按该命轮总使用次数计算</small></h4><div class="dtideSquadList">'+users+'</div></section><section class="dtideInsightPanel"><h4>Top 5 搭配命轮 <small>同一唤醒体主副手同时装备</small></h4><div class="dtideSquadList">'+partners+'</div></section></div></div>';
  }
