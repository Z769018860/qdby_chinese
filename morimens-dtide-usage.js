(()=>{
  ...
  function characterDetails(key){
    ...
    for(const {team} of mateRows){const members=team.members||[],target=members.find(m=>memberKey(m)===String(key));if(!target)continue;appearances++;if(team.token)bump(tokens,team.token);const seen=new Set();for(const m of members){const mk=memberKey(m);if(!mk||mk===String(key)||seen.has(mk))continue;seen.add(mk);bump(mates,{name:characterInfo(m).name,image:characterInfo(m).image})}}
    ...
    return {appearances,gearAppearances,teammates:finish(mates,appearances),wheels:finish(wheels,gearAppearances),covenants:finish(covenants,gearAppearances),tokens:finish(tokens,appearances).slice(0,3)};
  }
  ...
  function renderMatrix(){
    ...
    if(entity==='character'){
      ...
      groups=new Map(waves.map(w=>{const root=source[String(w)]||{},bucket=difficulty==='all'?(root.all||{}):(root.difficulties?.[difficulty]||root.all||{}),raw=entity==='wheel'?(bucket.wheels||[]):(bucket.covenants||[]);return {teamCount:Number(bucket.teamCount||0),items:raw.map(x=>({...x,key:String(x.key||x.id||x.name),name:itemName(x),image:localGearImage(entity,x.image),fallbackImage:x.image||''}))}}));
