(()=>{
  ...
  function characterDetails(key){
    ...
    return {appearances,gearAppearances,teammates:finish(mates,appearances),wheels:finish(wheels,gearAppearances),covenants:finish(covenants,gearAppearances),tokens:finish(tokens,appearances),creations:finish(creations,appearances)};
  }
  ...
  function renderMatrix(){
    ...
    if(entity==='character'){
      ...
      const source=(entity==='token'?usageStats:detailStats)?.waves||{};waves=Object.keys(source).map(Number).filter(Number.isFinite).sort((a,b)=>a-b);
      groups=new Map(waves.map(w=>{const root=source[String(w)]||{},bucket=difficulty==='all'?(root.all||{}):(root.difficulties?.[difficulty]||root.all||{}),raw=entity==='wheel'?(bucket.wheels||[]):entity==='covenant'?(bucket.covenants||[]):entity==='creation'?(bucket.creations||[]):(bucket.tokens||[]);return {teamCount:Number(bucket.teamCount||0),items:raw.map(x=>{const name=itemName(x),remote=x.image||(entity==='token'?tokenImage(name):'');return {...x,key:String(x.key||x.id||x.name),name,image:localGearImage(entity,remote)||remote,fallbackImage:localGearImage(entity,remote)?remote:''}})}}));
    ...
    const arrow=k=>k===sortKey?(asc?' ↑':' ↓'):' ↕',host=$('dtideMatrix'),label=entity==='character'?'角色':entity==='wheel'?'命轮':entity==='covenant'?'密契':entity==='token'?'钥令':'造物';if($('dtideMatrixTitle'))$('dtideMatrixTitle').textContent=label+'逐波出场率';if(!host)return;if(!rows.length){host.innerHTML='<div class="dtideEmpty">当前口径暂无'+label+'详细数据。</div>';return}
