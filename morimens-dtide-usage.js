(()=>{
  ...
  async function loadGearCatalog(){
    try{const [wheels,relics,covenants,assets]=await Promise.all([json('data/morimens/skeydb/public-v3/indexes/search-wheels.json'),json('data/morimens/skeydb/public-v3/indexes/search-relics.json'),json('data/morimens/skeydb/public-v3/indexes/search-covenants.json'),json('data/morimens/skeydb/public-v3/indexes/assets.json')]);for(const row of [...(wheels.records||[]),...(relics.records||[]),...(covenants.records||[])]){const entity=assets.entities?.[row.id],assetId=entity?.icon||`asset-${row.kind}-${String(row.id).replace(/^[^-]+-/,'')}-icon`,asset=assets.assets?.[assetId]||Object.values(assets.assets||{}).find(x=>x?.ownerId===row.id&&x?.slot==='icon'),path=asset?.availability?.path||asset?.availability?.candidates?.[0]||'',file=path.split('/').pop()?.split('?')[0]||(asset?.assetId?`${asset.assetId}.webp`:''),kind=row.kind;for(const name of [row.name,...(row.aliases||[])])if(name)gearByName.set(String(name).replace(/^"|"$/g,''),{file,kind})}}catch(e){console.warn('gear catalog unavailable',e)}
  }
  ...
  function characterDetails(key){
    ...
    const finish=(map,denom,filter=null)=>[...map.values()].map(x=>({...x,ratePct:denom?x.count/denom*100:0})).filter(x=>!filter||filter(x)).sort((a,b)=>b.count-a.count||String(a.name||a.key).localeCompare(String(b.name||b.key),'zh-CN')).slice(0,5),usefulCreation=x=>{const n=String(x.name||'').replace(/^"|"$/g,'').trim();return !fixedCreations.has(String(x.key||''))&&!/^(?:锈蚀钥匙|Rusted Key)$/i.test(n)};
