(()=>{
  ...
  function localGearImage(kind,url){const raw=String(url||''),file=raw.split('/').pop()?.split('?')[0];if(!file)return '';if(kind==='wheel')return `assets/morimens/wheels/${file}`;if(kind==='covenant')return `assets/morimens/covenants/Icon/${file.replace('_Box.webp','.webp')}`;if(kind==='creation')return raw.replace('/icon/','/thumb/icon/');return ''}
  function creationImage(name){for(const {team} of flatten(detailUsage?.records||[])){const token=team.token;if(token&&itemName(token)===name&&token.image)return String(token.image).replace('/icon/','/thumb/icon/')}return ''}
  ...
  function characterDetails(key){
    const mateRows=scopedRows(),gearRows=filteredDetailRows(),mates=new Map(),wheels=new Map(),covenants=new Map(),tokens=new Map();let appearances=0,gearAppearances=0;
    ...
    for(const {team} of mateRows){const members=team.members||[],target=members.find(m=>memberKey(m)===String(key));if(!target)continue;appearances++;if(team.token)bump(tokens,team.token,'creation');const seen=new Set();for(const m of members){const mk=memberKey(m);if(!mk||mk===String(key)||seen.has(mk))continue;seen.add(mk);bump(mates,{name:characterInfo(m).name,image:characterInfo(m).image})}}
    ...
    return {appearances,gearAppearances,teammates:finish(mates,appearances),wheels:finish(wheels,gearAppearances),covenants:finish(covenants,gearAppearances),tokens:finish(tokens,appearances)};
  }
  ...
  function renderMatrix(){
    ...
    if(entity==='character'){
      ...
      const source=(entity==='creation'?usageStats:detailStats)?.waves||{};waves=Object.keys(source).map(Number).filter(Number.isFinite).sort((a,b)=>a-b);
      groups=new Map(waves.map(w=>{const root=source[String(w)]||{},bucket=difficulty==='all'?(root.all||{}):(root.difficulties?.[difficulty]||root.all||{}),raw=entity==='wheel'?(bucket.wheels||[]):entity==='covenant'?(bucket.covenants||[]):(bucket.tokens||[]);return {teamCount:Number(bucket.teamCount||0),items:raw.map(x=>{const name=itemName(x),remote=x.image||creationImage(name);return {...x,key:String(x.key||x.id||x.name),name,image:localGearImage(entity,remote)||remote,fallbackImage:localGearImage(entity,remote)?remote:''}})}}));
  ...
  function bind(){if(bound)return;const ids=['dtideRankScope','dtideDifficulty','dtideWave','dtideClearType','dtideRateMode','dtideSort','dtideEntityType'];for(const id of ids)$(id)?.addEventListener('change',()=>setTimeout(renderAll,0));document.querySelector('.dtideLeaderboardTabs')?.addEventListener('click',e=>{const tab=e.target.closest('[data-dtide-entity]');if(!tab)return;const value=tab.dataset.dtideEntity;$('dtideEntityType').value=value;document.querySelectorAll('[data-dtide-entity]').forEach(x=>x.setAttribute('aria-selected',String(x===tab)));window.__dtideMatrixSort='total';window.__dtideMatrixAsc=false;renderMatrix()});$('dtideSeason')?.addEventListener('change',async()=>{try{await loadForSeason($('dtideSeason').value)}catch(e){console.warn('usage layer season load failed',e)}});bound=true}
  async function init(){
