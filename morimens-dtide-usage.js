(()=>{
  ...
  function wheelInsightHtml(d){
    const users=(d.users||[]).map((x,index)=>'<div class="dtideSquadRow"><span class="dtideSquadRank">'+(index+1)+'</span><div class="dtideChar">'+(x.image?'<img src="'+esc(x.image)+'" alt="" loading="lazy">':'')+'<span><b>'+esc(x.name)+'</b><small>'+x.count+' 次使用</small></span></div><div class="dtideSquadRate"><b>'+pct(x.ratePct)+'</b></div></div>').join('')||'<div class="dtideEmpty">当前详细样本暂无使用角色记录</div>';
    const partners=(d.partners||[]).map((x,index)=>'<div class="dtideSquadRow"><span class="dtideSquadRank">'+(index+1)+'</span><div class="dtideChar">'+(x.image?'<img class="dtideGearIcon" src="'+esc(x.image)+'" data-fallback="'+esc(x.fallbackImage||'')+'" referrerpolicy="no-referrer" loading="lazy" onerror="if(this.dataset.fallback&&this.src!==this.dataset.fallback){this.src=this.dataset.fallback;this.dataset.fallback=\'\'}else{this.hidden=true}" alt="">':'')+'<span><b>'+esc(x.name)+'</b><small>'+x.count+' 次同装</small></span></div><div class="dtideSquadRate"><b>'+pct(x.ratePct)+'</b></div></div>').join('')||'<div class="dtideEmpty">当前样本暂无主副手搭配命轮记录</div>';
