(()=>{
  ...
  function wheelDetails(key,fallback={}){
    const users=new Map(),targetKey=String(key),rows=filteredDetailRows();let totalUses=0,name=fallback.name||'',image=fallback.image||'',fallbackImage=fallback.fallbackImage||'';
    ...
    const top=[...users.values()].map(x=>({...x,ratePct:totalUses?x.count/totalUses*100:0})).sort((a,b)=>b.count-a.count||String(a.name).localeCompare(String(b.name),'zh-CN')).slice(0,5);
    return {key:targetKey,name:name||fallback.name||targetKey,image:image||fallback.image||'',fallbackImage:fallbackImage||fallback.fallbackImage||'',totalUses,users:top};
  }
