(()=>{
  ...
  async function loadGearCatalog(){
    try{const [wheels,relics,covenants,assets]=await Promise.all([json('data/morimens/skeydb/public-v3/indexes/search-wheels.json'),json('data/morimens/skeydb/public-v3/indexes/search-relics.json'),json('data/morimens/skeydb/public-v3/indexes/search-covenants.json'),json('data/morimens/skeydb/public-v3/indexes/assets.json')]);for(const row of [...(wheels.records||[]),...(relics.records||[]),...(covenants.records||[])]){const assetId=assets.entities?.[row.id]?.icon,asset=assets.assets?.[assetId],file=asset?.assetId?`${asset.assetId}.webp`:'';for(const name of [row.name,...(row.aliases||[])])if(name)gearByName.set(String(name).replace(/^"|"$/g,''),{file,kind:row.kind})}}catch(e){console.warn('gear catalog unavailable',e)}
  }
